...
adata = adata[adata.obs['batch'] == 'batch2', :].copy()
...
while sum(['_FINAL' not in x for x in adata.obs['current_leiden'].unique()]):
    ...
    dfs = Parallel(n_jobs=len(clusters_to_process)(delayed(meat_and_potatoes)(adata, cl, n_HVG) for cl in clusters_to_process)
    for i in dfs:
    ...
    adata.write_h5ad('input/03_norm/adata/adata_clustered_full_HVG1000_groupSize200_f0.75_categoricalNone_continuousNone.h5ad')
